...
if __name__ == '__main__':
    ...
    train_data_dir = 'images/ots_beta/'
    gps=3
    blocks=19
    net = Dehaze(3, 3)
    ...
    train_data_loader = DataLoader(TrainData(crop_size, train_data_dir), batch_size=train_batch_size, shuffle=True, num_workers=1)
    val_data_loader = DataLoader(ValData(val_data_dir), batch_size=val_batch_size, shuffle=False, num_workers=1)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(net.state_dict(), 'output/PSD-AECRNET-{}'.format(epoch))

        # --- Use the evaluation model in testing --- #
        net.eval()

        val_psnr, val_ssim = validation(net, 'AECRNet', val_data_loader, device, category)
